 ...
 * Handles loading, saving, deleting emails with Supabase
 */
...
const loadSavedEmails = useCallback(async () => {
  ...
  try {
    ...
    // For authenticated users, fetch emails from Supabase
    try {
      console.log('Fetching saved emails from Supabase...');
  ...
  const handleSaveEmail = useCallback(async () => {
    ...
    try {
      ...
      if (user) {
        ...
        // Update the email id with the one from Supabase
...
const testSupabaseConnection = useCallback(async () => {
  try {
    ...
    const { error } = await supabase.from('email_series').select('count');
